...
void CoinControlDialog::showMenu(const QPoint &point)
{
    ...
    if(item)
    {
        ...
        if (item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64 characters (this means its a child node, so its not a parent node in tree mode)
        {
            ...
            //if (model->isLockEcuadorDigitalCurrency(uint256(item->text(COLUMN_TXHASH).toStdString()), item->text(COLUMN_VOUT_INDEX).toUInt()))
            //{
...
/*void CoinControlDialog::updateLabelLocked()
{
    ...
    model->listLockEcuadorDigitalCurrencys(vOutpts);
    if (vOutpts.size() > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            /*if (model->isLockEcuadorDigitalCurrency(txhash, out.i))
            {
